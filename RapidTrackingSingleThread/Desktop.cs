...
namespace RapidTrackingSingleThread
{
    public class Desktop
    {
        ...
        {
            StringBuilder s = new StringBuilder();
            HtmlNodeCollection nds = node.SelectNodes(".//div[contains(@class,'hmHBZd')]");//27-02-2023
            foreach (HtmlNode nd in nds)
            {
                try
                {
                    ...
                    string price = nd.SelectSingleNode(".//span[contains(@class,'dv1Q3e')]|.//div[@class='YwF3uc']")?.InnerText.Trim() ?? "";//27-02-2023//22-02-2023
