...
namespace RapidTrackingMultithread
{
    class iOS
    {
        ...
        public string ProcessDocument(string seid, string keyword, HtmlDocument doc)
        {
            ...
            HtmlNodeCollection nodeCol = doc.DocumentNode.SelectNodes("//div[@id='rso']/div|//div[@id='rso']/g-card|//div[@id='taw']/div[@class='med']/div[2]/div|//div[@id='rso']/nav");
            ...
            if (string.IsNullOrEmpty(ndText.Trim()) || orgLinks == 0)
            {
                foreach (HtmlNode node in nodeCol)
                {
                    try
                    {
                        if (node.HasClass("kp-wholepage") || node.SelectNodes(".//div[contains(@class, 'kp-wholepage')]") != null)
                        {
                            //15-12-2020 swapped from bottom HtmlNodeCollection
        ...
        private string ProcessOrganic(HtmlNode node)
        {
            ...
            else
            {
                ...
                if (nds == null)
                    nds = node.SelectNodes(".//div[@class='KJDcUb']|.//div[@class='Lgnr0e J88qA vgnU9e BmP5tf']|.//g-card[@id='tscffb']|.//div[@class='mnr-c PHap3c']");//05-01-2021 //04-01-2021 missing classic link//18-12-2020 sitelinks missing selector //15-12-2020
                if (nds == null)
                        ...
                        else if (nd.Attributes["class"] != null || nd.Attributes.Count == 0)
                        {
...

                            {
                                ...
                                if (img != null)
                                {
                                    ...
                                    {
                                        //28-10-2019
                                        if (Regex.IsMatch(img.OuterHtml, "id=\"vidthumb\\d*\"") || nd.SelectSingleNode(".//div[@class='YgXj7b Qc4Zr']|.//div[contains(@class,'qW7zYd')]") != null || node.SelectSingleNode(".//div[@class='YgXj7b Qc4Zr']") != null || nd.SelectSingleNode(".//div[@class='hq7Nmd']") != null) //05-01-2021 //21-12-2020//16-12-2020 //14-12-2020
                                        {
        ...
        private string GetVideos(HtmlNode node)
        {
            ...
            if (nds == null)
                nds = node.SelectNodes(".//div[@jsname='ibnC6b']/a|.//div[@jscontroller='OmmTPc']|.//div[@class='QevLbc']/a|.//a[@class='OIDuRe']|.//div[@class='Q9mvUc']|.//div[@class='KJDcUb']"); // 14-12-2020//30-11-2020 videos item urls selectors //30-07-2020 //26-06-2020 included select for videos
        ...
        private string GetBlockType(HtmlNode node)
        {
            ...
            else
            {
                ...
                nd = node.SelectSingleNode(".//img[@alt='map image']|.//img[@alt='Affected area map']|.//img[contains(@alt,'Map of ')]|.//img[@alt='Affected area']|.//img[@alt='Immagine mappa']");//30-11-2020 //19-06-2020 // 13-03-2020 //01-05-2020
                if (nd != null)
                    return "Maps";
            }
