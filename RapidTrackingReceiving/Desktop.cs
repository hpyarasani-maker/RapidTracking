...
namespace Oxylabs_BulkKeywords
{
    public class Desktop
    {
        ...
        private string ProcessOrganic(HtmlNode node)
        {
            ...
            {
                HtmlNodeCollection nds = node.SelectNodes(".//div[@class='g']|.//div[@class='HD8Pae luh4tb cUezCb xpd O9g5cc uUPGi']|.//div[contains(@class,'g card-section')]");//11-01-2021 missing classic link//04-12-2020 videos block
                if (nds == null)
        ...
        private string GetRedirectedUrl(string url)
        {
            try //28-09-2020  try catch.
            {
                ...
                if ((url.StartsWith("https://") || url.StartsWith("http://") || url.StartsWith("ftp://")) && (!url.Contains("/aclk?") && !url.Contains("search?num=100"))) //28-08-2021  // 30-04-2020   // 30-04-2020 
                    return url;
            }
            catch (Exception ex)
            {
