...
namespace RapidTrackingSingleThread
{
    class OxylabsProxiesSeeMore
    {
        ...
        public string GetSeeMore(string html)
        {
            string url = string.Empty;
            HtmlDocument doc = new HtmlDocument();
            doc.LoadHtml(html);
            HtmlNode node = doc.DocumentNode.SelectSingleNode(".//h3/div[contains(@class,'GNJvt')]/span[@class='RVQdVd']");
            if (node != null)
            {
                node = doc.DocumentNode.SelectSingleNode(".//div/a[contains(@class,'T7sFge')]");
        ...
        public string[] GetTop100Mobile(string keyword, int seid, out string ip, string domain, string locale, string uule)
        {
            ...
            //26-05-2022
            // get Seemore url if existed.
            url = GetSeeMore(HTML);
            if (!string.IsNullOrEmpty(url.Trim()))
            {
                url = googleurl + domain + url.Trim().Replace("&Start=100", "");
                HTML = GetWebDataMobileSource(url, out ip);
            }
            //end 26-05-2022
