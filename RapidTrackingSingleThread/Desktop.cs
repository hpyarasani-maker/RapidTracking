    ...
    public class Desktop
    {
        ...
        public string ProcessDocument(string seid, string keyword, HtmlDocument doc, out int count)
        {
            ...
            if (string.IsNullOrEmpty(ndText) || orgLinks == 0)//08-04-2020
            {
                ...
                if (nodeCol == null)
                    nodeCol = doc.DocumentNode.SelectNodes("//div[@class='WvKfwe a3spGf']/div|//div[@class='WvKfwe a3spGf']/g-section-with-header|//div[@class='a3spGf WvKfwe']/div");  // 15-04-2020    //01-05-2020");  // 15-04-2020
                foreach (HtmlNode node in nodeCol)
                {
                    try
                    {
