...
using System.Xml;
using static RapidTrackingMultiThreadFirstPageResIPs.OxyResSearchParams;
...
namespace RapidTrackingMultiThreadFirstPageResIPs
{
    ...
    public async Task<ArrayList> GetHTML(string keyword, int seid, string jobid)
    {
        ...
        try
        {
            SearchProperties sp = OxyResSearchParams.searches.Where(s => s.seid == seid).SingleOrDefault();
            sp.seid = keyword;
            if (sp != null)
