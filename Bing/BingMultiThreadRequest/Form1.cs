...
namespace BingMultiThreadRequest
{
    public partial class Form1 : Form
    {

        string xmlPath1 = "C:\\inetpub\\wwwroot\\bing1.xml";
        string xmlPath2 = "C:\\inetpub\\wwwroot\\bing2.xml";
        string xmlPath3 = "C:\\inetpub\\wwwroot\\bing3.xml";
        ...
        {
            this.Text = "Bing_MultiThread_Request_(1-2-3)";//changes

            dateTimePicker1.Value = DateTime.Today;
            myDate = dateTimePicker1.Value.ToString("yyyy-MM-dd");
        ...
        private async Task StartProcess1()//16-02-2025
        {
            while (true)
            {
                string myDate = DateTime.Today.ToString("yyyy-MM-dd");
         
                string kwQry = "[GetKeywords_bing_1] '" + myDate + "'";

                await GetKeywords1(kwQry);

                if (worklist1.Items.Count <= 0)
                    break;

                foreach (string s in worklist1.Items)
                {
                    ...
                    try
                    {
                        ...
                        foreach (string[] src in alresult.Result)
                        {
                            ...
                            try
                            {
                                ...
                                await ProcessResults1(arRes, kw, int.Parse(seid), jobid);
                            }
                            catch (Exception ex)
                            {
                                try
        ...
        private async Task StartProcess2()//16-02-2025
        {
            while (true)
            {
                string myDate = DateTime.Today.ToString("yyyy-MM-dd");
               
                //string kwQry = "[GetKeywords_bing_Desktop] '" + myDate + "'"; 
                //string kwQry = "[GetKeywords_bing_Mobile] '" + myDate + "'";
                string kwQry = "[GetKeywords_bing_2] '" + myDate + "'";

                await GetKeywords2(kwQry);

                if (worklist2.Items.Count <= 0)
                    break;

                foreach (string s in worklist2.Items)
                {
                    string seid = s.Split(':')[0];
                    string kw = s.Split(':')[1];                    
                    try
                    {
                        ...
                        foreach (string[] src in alresult.Result)
                        {
                            ...
                            try
                            {
                                ...
                                await ProcessResults2(arRes, kw, int.Parse(seid), jobid);
                            }
                            catch (Exception ex)
                            {
                                try
                                {
                                    ...
                                    if (ex.Message == "Old page found.")
                                        isOldPage = true;
                                    await SendToDBFailure(kw, seid, jobid, isOldPage, "Bing Request Status is faulted");
                                }
                                finally { }
                            }
        ...
        private async Task StartProcess3()//16-02-2025
        {
            while (true)
            {
                string myDate = DateTime.Today.ToString("yyyy-MM-dd");
                ...
                //string kwQry = "[GetKeywords_bing_Mobile] '" + myDate + "'";
                string kwQry = "[GetKeywords_bing_3] '" + myDate + "'";



                await GetKeywords3(kwQry);

                if (worklist3.Items.Count <= 0)
                    break;

                foreach (string s in worklist3.Items)
                {
                    string seid = s.Split(':')[0];
                    string kw = s.Split(':')[1];
                    try
                    {
                        ...
                        foreach (string[] src in alresult.Result)
                        {
                            ...
                            try
                            {
                                ...
                                await ProcessResults3(arRes, kw, int.Parse(seid), jobid);
                            }
                            catch (Exception ex)
                            {
                                try
        ...
        public async Task ProcessResults1(ArrayList alRes, string kw, int seid, string jobid)
        {
            string qry = "";
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(result))
                    {
                        lblCount1.Invoke((MethodInvoker)(delegate ()
                        {
                            lblCount1.Text = "No. of Urls : " + alRes.Count;
                        }));
                        try
                        {
                            if (alRes.Count > 20)
        ...
        public async Task ProcessResults2(ArrayList alRes, string kw, int seid, string jobid)
        {
            string qry = "";
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(result))
                    {
                        lblCount2.Invoke((MethodInvoker)(delegate ()
                        {
                            lblCount2.Text = "No. of Urls : " + alRes.Count;
                        }));
                        try
                        {
                            if (alRes.Count > 20)
        ...
        public async Task ProcessResults3(ArrayList alRes, string kw, int seid, string jobid)
        {
            string qry = "";
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(result))
                    {
                        lblCount3.Invoke((MethodInvoker)(delegate ()
                        {
                            lblCount3.Text = "No. of Urls : " + alRes.Count;
                        }));
                        try
                        {
                            if (alRes.Count > 20)
        ...
        private async Task SendXmlToAPI1(string path)
        {
        ...
        private async Task SendXmlToAPI2(string path)
        {
            string submitURL = Common.readAPI();
        ...
        private async Task SendXmlToAPI3(string path)
        {
            string submitURL = Common.readAPI();
